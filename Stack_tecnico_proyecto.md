...
enriquece `nowPlaying.track.bpm` con esto).

**Actualización (2026-09-03): el cross-referencing real de `NextUpQueue` ya está
implementado** — `app/src/services/bpmQueue.ts` (`buildQueueForBpm`). Candidatos =
Liked Songs + canciones de las playlists marcadas en `PlaylistSelectionScreen`
(`getCachedLikedSongs()` + `getCachedTracksForSelectedPlaylists()`, ambas lecturas
puras de SQLite — sin red), BPM resuelto vía `getAudioFeaturesForTracks`. El mejor
match se agrega además a la cola real de Spotify (`spotifyStore.refreshQueueForBpm`
→ `enqueueBpmMatch`, usa `POST /me/player/queue`) cuando hay un dispositivo activo;
si no lo hay, se reporta como error visible en vez de tocar el playback de
cualquier otra forma (decisión explícita: no hacer skip/transferencia automática,
sólo encolar).

### Historial de reproducción — evitar repetir en la cola

**Nuevo (2026-09-03).** Con un BPM estable, ordenar siempre por cercanía hace que
la cola proponga las mismas 2-3 canciones una y otra vez. `app/src/store/playHistoryStore.ts`
guarda un historial acotado de IDs (`recentTrackIds`, más reciente primero) — el
tamaño es **configurable** vía `DEFAULT_PLAY_HISTORY_SIZE` (10 por defecto) o por
llamada (`recordPlayed(id, limit)`). Es un store de Zustand, no una tabla de SQLite:
es estado de la sesión de escucha en curso, no catálogo persistente, mismo criterio
que ya aplica a `bpm`/`queue`/`nowPlaying` en `sessionStore`/`spotifyStore`.

Se registra un track como "reproducido" en dos momentos:
- **Flujo real**: cuando `spotifyStore.refreshNowPlaying` confirma que Spotify
  cambió de canción, y también apenas se encola un match (`enqueueBpmMatch`) —
  esto último es proactivo, para no re-proponer/re-encolar la misma canción en el
  siguiente recálculo mientras Spotify todavía no llega a reproducirla.
- **Flujo simulado**: cuando `sessionStore` elige un nuevo "now playing" simulado
  (al iniciar sesión o cambiar de zona).

`app/src/utils/bpmMatch.ts` → `pickAvoidingRepeats(candidates, targetBpm, count, recentlyPlayedIds)`
es el punto único de esta lógica, usado tanto por `bpmQueue.ts` (real) como por
`mockPlayback.ts` (simulado): ordena por cercanía a BPM excluyendo lo reciente, y
sólo si eso deja menos candidatos "frescos" que `count` (catálogo o selección de
playlists chica) rellena con repetidos — se prefiere repetir algo a devolver una
