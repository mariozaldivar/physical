...
/**
 * El track del pool más cercano al BPM objetivo — para "now playing" al
 * iniciar sesión o cambiar de zona. Evita repetir lo último reproducido (ver
 * playHistoryStore.ts) cuando el pool da para elegir otra cosa.
 */
export function pickNowPlayingForBpm(targetBpm: number): Track {
  ...
  const [closest] = pickAvoidingRepeats(MOCK_TRACK_POOL, targetBpm, 1, recentlyPlayedIds);
 ...
 * — ver `services/bpmQueue.ts`, incluyendo evitar repetidos recientes.
 */
export function pickQueueForBpm(targetBpm: number, excludeTrackId?: string, count = 3): QueueTrack[] {
  const candidates = MOCK_TRACK_POOL.filter((track) => track.id !== excludeTrackId);
  const recentlyPlayedIds = usePlayHistoryStore.getState().recentTrackIds;
  return pickAvoidingRepeats(candidates, targetBpm, count, recentlyPlayedIds).map((track) => ({
    ...track,
    matchReason: bpmMatchReason(track.bpm ?? targetBpm, targetBpm),
  }));
}
