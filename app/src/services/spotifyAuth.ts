...
export async function completeWebLoginIfRedirected(): Promise<SpotifySession | null> {
  ...
  let state: string;
  try {
    ({ codeVerifier, state } = JSON.parse(raw) as { codeVerifier: string; state: string });
  } catch {
    throw new SpotifyAuthError("No se pudo completar el login: el estado guardado del intento anterior no es válido.");
  }
...
export async function getSession(): Promise<SpotifySession | null> {
  ...
  try {
    session = JSON.parse(raw) as SpotifySession;
  } catch {
    // Storage corrupto (raro, pero un raw JSON.parse crash acá tira toda la
    // pantalla de login) — se descarta y se trata como "sin sesión" en vez
    // de reventar.
    await clearSession();
    return null;
  }
