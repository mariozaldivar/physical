...
async function requestBatch(ids: string[], isRetry = false): Promise<ReccoBeatsAudioFeaturesResponse> {
  ...
  try {
    return JSON.parse(text) as ReccoBeatsAudioFeaturesResponse;
  } catch {
    throw new ReccoBeatsApiError(`ReccoBeats respondió ${response.status} con un cuerpo inesperado (no-JSON)`, response.status);
  }
}
