...
// analógico) mientras no esté presente. Permite arreglar la soldadura con la
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== Physical Band (Pulse Sensor Amped) ===");
  ...
  if (heartRateSensor.detect()) {
    Serial.println("Pulse Sensor detectado.");
  } else {
    Serial.println("Pulse Sensor NO detectado - se reintenta solo cada 3s.");
  }
}
...
void loop() {
  ...
  if (!heartRateSensor.isPresent() && now - lastSensorRetryAt >= kSensorRetryIntervalMs) {
    ...
    if (heartRateSensor.detect()) {
      Serial.println("Pulse Sensor detectado - midiendo.");
    }
  }
